...
def getData():
    with open("F:\\OPENPOSE\\openpose\\build\\workspace\\bone_dataSet.data", "r") as file:
        data = file.readlines()
    x, y = [], []
    for i in data:
        if i[0] != '#':
...
def train_net():
    ...
    model.eval().cuda()
    torch.save(model, 'F:\\OPENPOSE\\openpose\\build\\workspace\\23classification_model.pth')
    torch.save(model.state_dict(), "23classification2.pth")
